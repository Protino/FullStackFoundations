...
def editMenuItem(restaurant_id, menu_id):
	...
	if request.method == 'POST':
		...
		session.commit()
		flash(oldName + 'item successfully edited to '+editedItem.name)
		return redirect(url_for('restaurantMenu', restaurant_id = restaurant_id))
	else:
...
def deleteMenuItem(restaurant_id, menu_id):
	...
	print ('Deleting '+deletedMenuItem.name)
	if request.method == 'POST':
		session.delete(deletedMenuItem)
		session.commit()
		flash("Deleted menu item - "+deletedMenuItem.name)
		return redirect(url_for('restaurantMenu', restaurant_id = restaurant_id))
	else:
		return render_template('deletemenuitem.html', restaurant_id = restaurant_id, item=deletedMenuItem)

if __name__ == '__main__':
	app.secret_key = "super_secret_key :D :D"
	app.debug = True
